...
serve(async (req) => {
  console.log('Edge Function: list-bus-payments invoked.');
  ...
  try {
    if (!busId || typeof busId !== 'string' || busId.trim() === '') {
      console.error('Edge Function: Bus ID is missing, not a string, or empty.');
      return jsonResponse({ error: 'Bus ID is required and must be a non-empty string.' }, 400);
    }
    ...
    if (paymentsError) {
      console.error('Edge Function: Error fetching bus payments:', paymentsError.message);
      return jsonResponse({ error: 'Error fetching bus payments.' }, 500);
    }
